...
/// Turns a recipe into the two shareable forms she can send from the link panel:
/// a plain-text version tuned for SMS (numbered, highly legible) and a
/// self-contained "pretty page" HTML (champagne + diamonds) that opens standalone
/// in Safari from a text. Pure string-building, no SwiftUI — the App layer only
/// wires the results to ShareLink (mirroring how BudgetShare/BudgetXLSX split).
///
/// Recipe fields are freeform user/publisher text, so everything that lands in
/// markup is HTML-escaped here (the codebase had no escaper before this), and the
/// source URL is scheme-checked before it's ever turned into a link.
public enum RecipeShare {
    ...
    public static func text(name: String, serves: String, time: String,
        ...
        lines.append("")
        lines.append("— saved with love in Hannah's Kitchen ✦")
