...
/// the Notes page. Pure and in BloomCore so it can be unit-tested without a view.
///
/// The rule: if ANY line carries a bullet/number marker, only the marked lines
/// become items (she's writing prose with a list inside it); if NOTHING is
/// marked, every non-empty line is an item (she just typed a plain list).
