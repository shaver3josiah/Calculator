...
struct RealEstateCard: View {
        ...
        Card {
            VStack(alignment: .leading, spacing: 16) {
                ...
                ProjectionFieldRow(
                    ...
                    leftText: $d.realEstate.currentValue,
                    rightLabel: "Growth %/yr",
                    rightText: $d.realEstate.rate
                )
                yearsSlider
                ProjectionFormField(label: "Net rental yield %/yr (optional)", text: $d.realEstate.netYield)
                Text(isLand
    ...
    private var isLand: Bool { drafts.realEstate.kind == "land" }
    ...
    private func kindChoice(_ id: String, label: String, rate: Double) -> some View {
        let selected = drafts.realEstate.kind == id
        return Button {
            withAnimation(BloomMotion.springSoft) {
                drafts.realEstate.kind = id
                drafts.realEstate.rate = Formatters.plain(rate)
            }
        } label: {
            Text(label)
                .font(bloomBody(13, weight: selected ? .semibold : .medium))
    ...
    private var yearsSlider: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Slider(
                value: Binding(
                    get: { Double(years) },
                    set: { drafts.realEstate.years = String(Int($0)) }
                ),
                in: 1...40, step: 1
            )
    ...
    private var logButton: some View {
        Button {
            historyStore.add(
                ...
                extra: [
                    "value": drafts.realEstate.currentValue,
                    "ratePct": drafts.realEstate.rate,
                    "years": String(years)
                ]
            )
        ...
        .buttonStyle(.plain)
        .discoverable("tools.realEstateLog", cornerRadius: 12)
    }
}
