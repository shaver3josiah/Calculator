...
/// Foil brick: silver block crossed by a single pink center band.
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            let band = Path(roundedRect: CGRect(x: w*0.14, y: h*0.44, width: w*0.72, height: h*0.12),
                            cornerRadius: w*0.02)
            ctx.solid(band, Pack2Palette.labelPink, lw)
            var sheen = Path()
...
/// Squat round tub: gently tapered body under an overhanging pink lid rim.
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            let lid = Path(roundedRect: CGRect(x: w*0.22, y: h*0.32, width: w*0.56, height: h*0.14),
                           cornerRadius: w*0.04)
            ctx.solid(lid, Pack2Palette.labelPink, lw)
            var sheen = Path()
