...
private enum Pack1Palette {
    ...
    static let labelPink = Color(red: 0.94, green: 0.47, blue: 0.66)
...
// MARK: - 2. Sugar — cream bag, crimped zigzag top, pink label
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            let label = Path(roundedRect: CGRect(x: w * 0.3, y: h * 0.44, width: w * 0.4, height: h * 0.28),
                             cornerSize: CGSize(width: w * 0.05, height: w * 0.05))
            ctx.fill(label, with: .color(Pack1Palette.labelPink))
            ctx.stroke(label, with: .color(Pack1Palette.stroke), lineWidth: lw * 0.8)
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            let box = Path(roundedRect: CGRect(x: w * 0.28, y: h * 0.12, width: w * 0.44, height: h * 0.78),
                           cornerSize: CGSize(width: w * 0.04, height: w * 0.04))
            ctx.fill(box, with: .color(Pack1Palette.labelPink.opacity(0.85)))
...
struct YeastArt: View {
    var body: some View {
        Canvas { ctx, size in
            ...
            packet.closeSubpath()
            ctx.fill(packet, with: .color(Pack1Palette.labelPink.opacity(0.85)))
