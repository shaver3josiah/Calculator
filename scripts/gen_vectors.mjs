...
const BLOOMCORE_RES = path.join(ROOT, "Packages", "BloomCore", "Sources", "BloomCore", "Resources");
...
mkdirSync(BLOOMCORE_RES, { recursive: true });
writeFileSync(path.join(BLOOMCORE_RES, "foods.json"), JSON.stringify(foodsResult.unified, null, 2) + "\n", "utf-8");

const eggsJson = buildEggsJson();
writeFileSync(path.join(BLOOMCORE_RES, "eggs.json"), JSON.stringify(eggsJson, null, 2) + "\n", "utf-8");

const fundsDefault = FUNDS_RAW.map((f) => ({ name: f.name, ratePct: f.rate }));
writeFileSync(path.join(BLOOMCORE_RES, "funds-default.json"), JSON.stringify(fundsDefault, null, 2) + "\n", "utf-8");
