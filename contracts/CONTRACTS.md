...
`StoreKey` gains `case stewardship = "bloom_stewardship"` **(2026-07-15, amendment: this key already ships in `Packages/BloomCore/Sources/BloomCore/Persistence.swift`, backing `BudgetStore`'s stewardship settings, but was never documented in v1, v2, or v3; recorded here so the registry matches shipped code. The frozen block above is unchanged.)**
...
> Status, 2026-07-15: the v3 items below are NOT implemented as of Bloom v0.1.36. The five-tab registry, `MoreView`, `ShareCodec`, `FundDefaults`, the `ShopList`/lists migration, and `StoreKey` cases `lists`/`keychords` do not exist in shipped code. The shipped tab registry remains the v2 seven-tab set (`BloomTab`: calc, proj, lists, kitchen, tools, budget, music), and keychords persist via a private raw filename (`"bloom_keychords"` in `App/Views/Music/MusicStore.swift`) that bypasses `StoreKey`. The frozen v3 text below is unchanged.
...
- **D10**, Liquid Glass is deferred to v1.1. No v1.0 code should adopt `.glassEffect` or related Liquid Glass materials; this matches the Parity Matrix Parking Lot entry already recorded by the docs worker (`docs/parity/PARITY_MATRIX.md`). **Partially superseded by D13 (2026-07-31)** — see below.
- **D13**, Liquid Glass on the keypad only (2026-07-31, requested by Josiah, narrowing D10). The calculator key faces adopt `.glassEffect` via the shared `bloomKeyGlass` helper in `App/Components/KeypadButton.swift`, gated by `#if compiler(>=6.2)` **and** `#available(iOS 26.0, *)`, with the existing flat fill as the fallback — the iOS 17.0 deployment target and CI's floating "newest Xcode on `macos-latest`" toolchain (`.github/workflows/build-check.yml:12-19`) both stay green. The rest of D10 still stands: cards, sheets, and the tab bar remain non-glass for v1.0.
...
- **D14**, budget reorder, give-first default, and the pre-26 key face (2026-08-12, requested by Josiah). Three additions:
  1. `BudgetMath` gains `public static func move<T>(_ array: [T], from: Int, to: Int) -> [T]` — an additive extension to the frozen block above; every signature in it is unchanged. Drag-and-drop semantics: `to` is the element's FINAL resting index in either direction (not a neighbour swap), out-of-range indices return the array unchanged. Covered by `BudgetTests.testMove`.
  2. The budget tab's category and row reordering moves from the paired up/down arrow menu to a press-and-drag grip handle (`App/Views/Budget/CategoriesSection.swift`). While a card is plucked the whole stack folds to a uniform deck of headers so the slot pitch is constant; placement fires the existing `PetalBurstView` confetti, gated behind `theme.petalsOn` and Reduce Motion like every other petal surface. `BudgetStore.reorderCategory/reorderRow(direction:)` are replaced by `moveCategory(from:to:)`/`moveRow(category:from:to:)`. VoiceOver keeps discrete Move up / Move down actions, so the arrow affordance is removed visually, not functionally.
  3. D13's "with the existing flat fill as the fallback" is amended: the pre-iOS-26 key face is now a hand-drawn glass REPLICA (translucent tint, top sheen, specular hairline border, soft lift) in `bloomGlassReplicaFace`, not a flat fill. It uses gradients only — no `.glassEffect`, no `Material`, no backdrop blur — so D10's "no v1.0 code should adopt Liquid Glass materials" still holds literally on the pre-26 path while the two paths finally look like one keyboard. The rest of D10 stands: cards, sheets, and the tab bar remain non-glass for v1.0.
- **D15**, persistence timing and the Int conversion guard (2026-08-12, from a 28-agent robustness audit of the shipped v0.1.47). Two additive BloomCore APIs; no frozen signature changes:
  1. `JSONStore.set` is no longer synchronous. It encodes on the store's existing serial queue, coalesces to the newest value per key, and writes 500ms after the last call; a failed write returns to the queue and retries. `JSONStore` gains `public func flush()`, a synchronous write-through wired to `scenePhase != .active` in `App/BloomApp.swift` alongside the existing `draftStore.flush()`. Rationale: every one of the 24 call sites is a store `didSet` on the main thread, and v0.1.47's per-keystroke `DecimalField` turned that into a whole-database encode and atomic write per character. `get` reads the pending value before disk, so read-your-writes still holds; the corrupt-file quarantine still guards only the disk path. Covered by `PersistenceTests`.
  2. New file `Packages/BloomCore/Sources/BloomCore/SafeInt.swift` adds `Double.clampedInt(to:fallback:)` and `String.clampedInt(to:fallback:)`. `Int(_: Double)` is precondition-checked and traps on NaN, infinity, and anything outside Int64; eight sites converted free text she typed. The worst (`WholeLifePanel` "Years paying") was a persisted draft replayed by `.onAppear`, so one mis-tap crashed the Projection tab on every launch permanently, with "Clear this page" unreachable below the crashing panel. Covered by `SafeIntTests`. `MathModes.fraction`/`simplify`/`gcd` were hardened against the same family in `Int` (cross-multiply overflow, `abs(Int.min)`, negating `Int.min`); public signatures unchanged, unrepresentable results now take the existing `nil` path.
  3. ~~Known divergence left in place: `CalcEngine.setOp` writes the literal string "NaN" ...~~ **Superseded by D16 (2026-08-12): the claim was wrong and the bug is fixed.**
- **D16**, calculator chained-operator guard and an App-layer test target (2026-08-12).
  1. `CalcEngine.setOp` now applies the same `isFinite` guard `equals` has always had: a non-finite intermediate resets `current` to "0" and clears `stored`. Previously a chained `5 / 0 +` wrote the literal string "NaN" into the buffer, and every later operand parsed it back out, so the calculator stayed poisoned until AC. This is a **deliberate divergence from the source HTML**, whose own `setOp` has the same omission (`calc.current=plain(res)` with no guard, against a guarded `equals`). D15.3 claimed `contracts/vectors.json` pinned the unguarded behavior; it does not - zero of the 28 `calc` vectors exercise a non-finite chain, and all 28 still pass. Covered by `CalcEngineTests`.
  2. New `BloomAppTests` target (XcodeGen `bundle.unit-test`, hosted by the Bloom app, in the Bloom scheme) plus a simulator `xcodebuild test` step in `build-check.yml`. `Packages/BloomCore` keeps running under `swift test` on Linux; this target exists for App-layer logic that imports SwiftUI and therefore cannot. First coverage: `NotesArchiveStore.stash` (including the blank-page guard whose failure mode is deleting the note it protects) and `BudgetStore.replacementSummary`/`previewImport`, which decide whether the budget-import confirmation appears at all.
  3. The v1 style rule "files under 400 lines" is stale: 12 files already exceed it, the largest at 909. Recorded rather than enforced piecemeal; splitting only recently-touched files would be churn without benefit.
