...
- **D15**, persistence timing and the Int conversion guard (2026-08-12, from a 28-agent robustness audit of the shipped v0.1.47). Two additive BloomCore APIs; no frozen signature changes:
  ...
  3. Known divergence left in place: `CalcEngine.setOp` writes the literal string "NaN" into the buffer on a chained divide-by-zero, where `equals` guards `isFinite`. Fixing it would diverge from the JS source that `contracts/vectors.json` pins, and AC clears it. Recorded, not fixed.
