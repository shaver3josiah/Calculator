...
This is the one time checklist to get Bloom from this repo onto TestFlight. You are on Windows. All Apple specific work (building, signing, uploading) happens on GitHub's macOS runners, not on your machine. You only need a web browser and a terminal that can run git.
...
2. Register the bundle ID.
   ...
   5. Description: `Bloom`.
   6. Bundle ID: choose Explicit, enter `com.shaver.bloomcalculator`.
...
3. Create the App Store Connect app record.
   ...
   4. Name: `Bloom Calculator` (this is the App Store listing name, it can differ from the on device display name `Bloom`).
   5. Primary language: English (U.S.) or your preference.
   6. Bundle ID: select `com.shaver.bloomcalculator` from the dropdown, it should already appear since you registered it in step 2.
   7. SKU: any unique string, for example `bloomcalculator001`.
...
6. Add Hannah as a user and internal tester.
   1. In App Store Connect, click Users and Access.
   2. Click the Users tab (or People, depending on account type).
   3. Click the plus button to invite a new user.
   4. Enter Hannah's name and email address.
   5. Role: Developer or App Manager is enough for her to see builds and TestFlight, avoid Admin unless you want her managing the whole account.
   6. Under app access, grant access to the Bloom Calculator app specifically.
   7. Click Invite. Hannah gets an email to accept and set up two factor authentication if she does not already have an Apple Developer account association.
   8. Once she accepts, go to your app, click the TestFlight tab.
   9. Click Internal Testing in the sidebar (or App Store Connect Users group under Internal Testing).
   10. Click the plus button next to Testers, select Hannah's name from the list of users with app access, click Add.
...
1. Create a private repository.
   ...
   3. Repository name: `bloom-calculator` or similar.
...
Run these from a Windows terminal (PowerShell or Git Bash) inside the `BloomCalculator` folder.

1. Connect the local folder to your new GitHub repo.
   ```
   git init
   git add .
   git commit -m "Initial Bloom scaffold"
   git branch -M main
   git remote add origin https://github.com/shaver3josiah/Calculator.git
...
4. Find the build in TestFlight.
   1. Go to App Store Connect, open Bloom Calculator, click the TestFlight tab.
   ...
   3. Once processing finishes, the build appears under Internal Testing and Hannah's TestFlight app will offer it automatically (assuming section A.6 is done).
...
(CA92.1). BloomCore's persistence type is `JSONStore`, which is initialized with an
